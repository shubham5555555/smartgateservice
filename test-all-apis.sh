...
# Complete API Test Script with Default OTP
...
echo "=========================================="
echo "Complete API Test - Smart Gate Backend"
echo "Using default OTP: 123456 for $PHONE_NUMBER"
echo "=========================================="
echo ""
...
# Test 2: Verify OTP
echo "2. Testing Verify OTP (using default OTP: $DEFAULT_OTP)..."
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/auth/verify-otp" \
    -H "Content-Type: application/json" \
    -d "{\"phoneNumber\": \"$PHONE_NUMBER\", \"otp\": \"$DEFAULT_OTP\"}")
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT "$BASE_URL/users/profile" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"fullName": "Test User", "email": "test@example.com", "role": "Owner", "block": "A", "flat": "101"}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/staff" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"name": "John Doe", "role": "Driver", "phoneNumber": "9876543210"}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/parking/apply" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"description": "Permanent Sticker Request", "parkingType": "Permanent", "vehicle": "Mercedes Benz S-Class", "licensePlate": "MH 43 SA 1139", "block": "A"}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/visitors" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"name": "Test Visitor", "type": "Guest", "phoneNumber": "9876543210", "isPreApproved": false}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
